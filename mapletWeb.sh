...
path="/Library/Server/Web/Data/Sites/Default/dataX/"
if [ -e $path ];
then
   path="/Library/Server/Web/Data/Sites/Default/dataX/"
   #echo "  Machine:  Server "
else
   path="/Library/WebServer/Documents/dataX/"
   #echo "  Machine:  Local "
fi
...
if [ ! -e MAPFILES/${id}X.MAP ];
then
	echo "###########################"
	echo "###########################"
   echo 	MAPFILES/${id}X.MAP does not exist
   exit
fi
...
grep P6 LMKFILES/${id}X.LMK | cut -c 1-12 > tmp
grep -f tmp PICINFO.TXT > $path/$id/imgInfo.txt

echo ${id}X.LMK | cut -c 1-6 > tmp
stationEval.sh tmp
...
if [ "$2" != "n" ]
then
	echo "<tr><td><a href=/dataX/$id/maplet.html?id=$id>$id</a></td>   <td>$newGSD</td>    <td>$Q</td>   <td>$coord</td>     <td>$avg</td>   <td>$sigX</td>    <td>$maxAlb</td>    <td>$stdAlb</td>  <td> $d</td>    <td>$w</td></tr>" >> $path/mapletList.html
#else 
#	echo "<tr><td><a href=/dataX/$id/maplet.html?id=$id>$id</a></td>   <td>$newGSD</td>    <td>$Q</td>   <td>$coord</td>    <td>$sig</td>   <td>$avg</td>   <td>$sigX</td>    <td> $d</td></tr>" 
fi
