...
c     Change into physical units
      DO J=-QSZ,QSZ
        DO I=-QSZ,QSZ
          HT1(i,j) = HT1(i,j) * 1000 * SCALE
          HT2(i,j) = HT2(i,j) * 1000 * SCALE
        ENDDO
      ENDDO
c     Normalize by getting the average and subtracting
      delta = HT1(-QSZ+1,-QSZ+1) - HT2(-QSZ+1,-QSZ+1)
      write (*,*) "Height: ", HT1(-QSZ+1,-QSZ+1), HT2(-QSZ+1,-QSZ+1)
      write (*,*) "delta: ", delta

      DO J=-QSZ,QSZ
        DO I=-QSZ,QSZ
          HT1(i,j) = HT1(i,j) - delta
        ENDDO
      ENDDO
...
240   format (e13.5, 4000(", ", e13.5))
      write (*,*) "myDate=", map1 ;
      write (*,*) "rms='", sqrt (sumError), "'"
      write (*,*) "#Min is: ", myMin
      write (*,*) "#Max is: ", myMax

      write (10,*) "#RMS is: ", sqrt (sumError)
