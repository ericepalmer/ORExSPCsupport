...
13      CONTINUE
      ...
      WRITE(6,*) 'pixlinpic (p) or ltdlng (l) or map p/l (m)?'
      READ(5,FMT='(A1)') ANS
      WRITE(XLINE(1),FMT='(A1,79X)') ANS

C   Experimental choice I:
     ...
     .                KMAT(2,1), KMAT(2,2), KMAT(2,3)
        ...
        WRITE(6,*) 'scale, qsz, seed, max scale'
        READ(5,*) S0, QSZ, SEED, MXSCL
        WRITE(6,*) 'Input 6 character map name'
        READ(5,FMT='(A6)') BIGMAP
        GO TO 160
      ENDIF

C   Picture location determines map center location V:

      IF ((ANS.EQ.'P').OR.(ANS.EQ.'p')) THEN
        WRITE(6,*) 'Input picture name'
        READ(5,FMT='(A12)') PICNM
        WRITE(XLINE(2),FMT='(A12,68X)') PICNM
        WRITE(6,*) 'Input patch center (imgpx,imgln)'
        READ(5,*) IMGPL(1), IMGPL(2)  
        WRITE(XLINE(3),*) IMGPL(1), IMGPL(2)  
...
C   Lat/Lon determines map center location V:
...
C   Map location determines map center location V:
      ...
      IF ((ANS.EQ.'M').OR.(ANS.EQ.'m')) THEN
        WRITE(6,*) 'Input 6 character map name'
        READ(5,FMT='(A6)') NAME
        WRITE(XLINE(2),FMT='(A6,74X)') NAME
        LMRKFILE='./MAPFILES/'//NAME//'.MAP'
        CALL READ_MAP(LMRKFILE,BTMP,QSZ,S0,V,UX,UY,UZ,ZHT,NH)
        DO I=-QSZ,QSZ
        DO J=-QSZ,QSZ
          IF(NH(I,J).LE.(0.005)) ZHT(I,J)=0
        ENDDO
        ENDDO
        ...
        IF((ABS(Z1).LE.QSZ).AND.(ABS(Z2).LE.QSZ)) THEN
          Z3=ZHT(NINT(Z1),NINT(Z2))
        ENDIF
        V(1)=V(1)+S0*(Z1*UY(1)+Z2*UX(1)+Z3*UZ(1))
        V(2)=V(2)+S0*(Z1*UY(2)+Z2*UX(2)+Z3*UZ(2))
        V(3)=V(3)+S0*(Z1*UY(3)+Z2*UX(3)+Z3*UZ(3))
        GO TO 150
      ENDIF
...
C   Input bigmap scale, size, random seed and max maplet scale
C   Input bigmap name

      WRITE(6,*) 'scale, qsz, seed, max scale,' 
      WRITE(6,*) '[optional cw map rotation (deg)]'
      READ(5,FMT='(A80)') LINE
      LINE(79:80)='0 '
      READ(LINE,*) S0, QSZ, SEED, MXSCL, ROT
      WRITE(6,*) 'Input 6 character map name'
      READ(5,FMT='(A6)') BIGMAP
...
C   Determine input maplet list:
      ...
      IF(LMKLX) THEN
        INFILE='LMRKLISTX.TXT'
        BFLG=.TRUE.
      ELSE
        INFILE='BIGMAP.IN'
        INQUIRE(FILE=INFILE, EXIST=BFLG)
        IF(.NOT.BFLG) THEN
          INFILE='LMRKLISTR.TXT'
        ENDIF
        INQUIRE(FILE=INFILE, EXIST=BFLG)
        IF(.NOT.BFLG) THEN
          INFILE='LMRKLIST.TXT'
...
C   Open maplet list files:
      ...
      OPEN(UNIT=70,FILE='USED_MAPS.TXT')
      OPEN(UNIT=71,FILE='INSIDE.TXT')

C   Determine maplet height, albedo and slope contributions:

      SMAX=0
      KPIC=0
      OPEN(UNIT=69,FILE=INFILE,STATUS='OLD')
10      CONTINUE
        IF(.NOT.LMKLX) THEN
          READ(69,FMT='(A6)') NAME
          ...
          ENDIF
        ELSE
          READ(69,FMT='(A80)') LINE
          IF(LINE(1:3).EQ.'END') GO TO 30
          READ(LINE(7:80),*) K0,J,(V1(K), K=1,3),S1
          CALL VSUB(V,V1,W)
          IF(VNORM(W).GT.2*(S0*QSZ+S1*K0)) GO TO 10
          NAME=LINE(1:6)
        ENDIF
        FLAG=.FALSE.
        EFLG=.TRUE.
        ...
        IF(VDOT(UZ,U1Z).LT.(0.250)) GO TO 10
        CALL VSUB(V,V1,W)
        IF(VNORM(W).GT.2*(S0*QSZ+S1*K0)) GO TO 10
        ...
        DO J0=-K0,K0-1
          ...
          DO I1=0,1
            ...
            W(2)=V1(2)-V(2)
            W(3)=V1(3)-V(3)
            IF(ABS(VDOT(UZ,W)).GT.TOL) GO TO 10
...
20        CONTINUE
        ...
        IF(FLAG) WRITE(70,FMT='(A8)') NAME//COVERED      ! Record maplets used
        IF(EFLG) WRITE(71,FMT='(A6)') NAME               ! Record maplets completely inside
        LMRKFILE='./LMKFILES/'//NAME//'.LMK'

C   Determine images used:

        OPEN(UNIT=10, FILE=LMRKFILE, STATUS='OLD')
25        CONTINUE
          READ(10,FMT='(A80)') LINE
          IF(LINE(1:8).NE.'PICTURES') GO TO 25
26        CONTINUE
          READ(10,FMT='(A80)') LINE
          IF(LINE(1:12).NE.'MAP OVERLAPS') THEN
            DO K=1,KPIC
              IF(LINE(1:12).EQ.PICID(K)) GO TO 26
            ENDDO
            KPIC=KPIC+1
            PICID(KPIC)=LINE(1:12)
            GO TO 26
          ENDIF
        CLOSE(UNIT=10)
...
30    CONTINUE
      ...
      WRITE(70,FMT='(A3,3X)') 'END'
      WRITE(71,FMT='(A3,3X)') 'END'

      CLOSE(UNIT=71)
      CLOSE(UNIT=70)

      INFILE='USED_MAPS.TXT'
      CALL PIXINLMX(INFILE)
...
300       CONTINUE
          K=K+1
          READ(66,FMT='(A80)') XLINE(K)
          IF(XLINE(K)(1:3).NE.'END') GO TO 300
        CLOSE(UNIT=66)
        J=K-1
        OPEN(UNIT=66,FILE='SIGMAS.TXT',STATUS='UNKNOWN')
          DO K=1,J
            WRITE(66,FMT='(A80)') XLINE(K)
          ENDDO
          WRITE(66,FMT='(A6, 2F24.6,26X)') BIGMAP, Z5, Z1/Z2
          WRITE(66,FMT='(A3,77X)') 'END'
        CLOSE(UNIT=66)
      ENDIF

C   Make SIGMAS.pgm file:

      OPEN(UNIT=66, FILE='SIGMAS.DAT', ACCESS='DIRECT',
     .     RECL=2*QSZ+1, STATUS='UNKNOWN')
        DO J=-QSZ,QSZ
          DO I=-QSZ,QSZ
            IF(SUSE(I,J)) THEN
              Z1=(ZHT2(I,J)-Z4)/(Z5-Z4)
              OUTLINE(I+QSZ+1:I+QSZ+1)=CHAR(1+NINT(254*Z1))
            ELSE
              OUTLINE(I+QSZ+1:I+QSZ+1)=CHAR(0)
            ENDIF
          ENDDO
          WRITE(66,REC=J+QSZ+1) OUTLINE(1:2*QSZ+1)
        ENDDO
      CLOSE(UNIT=66)
      ...
      WRITE(6,*) Z4, Z5, IX+QSZ+1, JX+QSZ+1
      WRITE(6,*) 'gc SIGMAS.pgm'           
