C     ftn  display_landmark.f /Users/rgaskell/toolkit/lib/spicelib.a -o display_landmark.e

c  Notes:  This displays a bigmap's field of view with a specific landmark
c       highlighted.  It makes it easier to locate where landmarks are on
c       a bigmap.  Based on map_coverage.f

c  2013-02-27.  Version 1
...
C  You can use command line or get prompted
      ...
      if (argc .EQ. 2) then
        ...
        bigmap = val
        if (bigmap .EQ. '-h') then
          write (*,*) "display_landmark.e <BIGMAP> <LANDMARK>"
          stop
        endif
        call getArg (2, val)
        search = val
...
10      CONTINUE
        ...
        if (name .NE. search) goto 10   ! skip the files we don't want
...
30    CONTINUE
      ...
      write (*,*) bigmap, " Corner reference " 
      write (*,*) "Center: ", (xMax - xMin)/2+xMin, (yMax - yMin)/2+yMin
      write (*,'(20x, A5, 20x)') "YMin"
      write (*,'(20x, I5, 20x)') yMin
      write (*,'(13x, 5x, A)') "+-----------+"
      write (*,'(13x, 5x, A)') "|           |"
      write (*,'(12x, a5, a, a5)') "xMin", " |           | ", "xMax"
      write (*,'(12x, i5, a, i5)') xMin, " |           | ", xMax
      write (*,'(13x, 5x, A)') "|           |"
      write (*,'(13x, 5x, A)') "|           |"
      write (*,'(13x, 5x, A)') "+-----------+"
      write (*,'(20x, I5, 20x)') yMax
      write (*,'(20x, A5, 20x)') "YMax"
...
c      WRITE(6,*) 'xv coverage_m.pgm &'           

      open(unit=55,file='coverage.gray', status='unknown')
      close(unit=55, status='delete')

      write (*,*), "gc found.pgm"
c      call system ("convert coverage_m.pgm coverage_m.jpg")
