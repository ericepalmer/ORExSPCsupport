...
c  1.2 - 24 July 15 -- normalized the data at point 1,1 -- simple subtraction
c  1.3 - 23 Aug 15 -- calculated the radius vector and normalized
c  1.4 - 25 Jan 16 -- Fixed the RMS value 
      ...
      REAL*4                maxVal
      real*4                sumError, rms
      real                  avg1, avg2, sum1, sum2, delta
      ...
      write (*,*) "Units in meters"
      CALL READ_MAP(MAP1,NTMP,QSZ,SCALE,V,UX,UY,UZ,HT1,AL1)
      width = QSZ
c      write (*,*) "Width/Height ", QSZ*2
      write (*,'(1X, A,F8.5)') "Scale1:  ", SCALE * 1000
      radius1 = sqrt (v(1)**2 + v(2)**2 + v(3)**2) * 1000
      write (*,*) "Radius 1:", radius1

      CALL READ_MAP(MAP2,NTMP,QSZ,SCALE,V,UX,UY,UZ,HT2,AL2)
c      write (*,*) "Width/Height ", QSZ*2
      write (*,'(1X, A,F8.5)') "Scale2:  ", SCALE * 1000
      radius2 = sqrt (v(1)**2 + v(2)**2 + v(3)**2) * 1000
      write (*,*) "Radius 2:", radius2
...
c     Change into physical units
      DO J=-QSZ,QSZ
        DO I=-QSZ,QSZ
          HT1(i,j) = HT1(i,j) * 1000 * SCALE + radius1
          HT2(i,j) = HT2(i,j) * 1000 * SCALE + radius2
        ENDDO
      ENDDO

c     Normalize by getting the average and subtracting
c      delta = radius1 - radius2
c      write (*,*) "delta: ", delta

c      DO J=-QSZ,QSZ
c        DO I=-QSZ,QSZ
c          HT1(i,j) = HT1(i,j) - delta
c        ENDDO
c      ENDDO
...
240   format (e13.5, 4000(", ", e13.5))
      write (*,*) "# Map1 Used: ", map1
      write (*,*) "# Map2 Used: ", map2
