...
c  1.3 Apparently, I'm a moring and didn't report slope/tilt in % (multiple by 100)
...
c      LMRKFILE=MAP0(1:i-4)//'.TXT'
      ...
      write (*,*) "Slope output file: ", LMRKFILE
...
c     Each delta is the slope in the x or y position.  We get the distance
      DO J=-QSZ+1,QSZ
        DO I=-QSZ+1,QSZ
          dx = ht0(i-1,j) - ht0(i,j)
          dy = ht0(i,j-1) - ht0(i,j)
          slope (i, j) = sqrt (dx*dx + dy*dy) * 100
        ENDDO
      ENDDO

c     Write the slope file
      DO J=-QSZ,QSZ
        write(10,240) ( slope(I,J), I=-QSZ,QSZ )
      ENDDO
...
c ---- Calculate average
      ...
      DO J=-QSZ+1,QSZ
        DO I=-QSZ+1,QSZ
          if (maxVal .LT. slope (i,j)) maxVal = slope (i,j)
          if (minVal .GT. slope (i,j)) minVal = slope (i,j)
          sum = sum + slope (i,j)
          count = count+1
        ENDDO
      ENDDO
