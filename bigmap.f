...
C     2015-09-01   Increased significant digits for SIGMAS.TXT
C                   
...
C   Orient frame with horizontal axis to the East:
      ...
      DO K=1,3
        V(K)=V(K)+Z1*S1*UZ(K)
      ENDDO
      CALL PATCH_COORDS(NTMP,K0,MUSE,ZH,UX,UY,UZ)
      CALL ORIENT(UX,UY,UZ)
      CALL GET_HEIGHTS(NTMP,K0,UX,UY,UZ,V,S1, MUSE,ZH,AL)
      Z1=ZH(0,0)
      DO I=-K0,K0
...
20        CONTINUE
        ...
        ENDDO
        ENDDO
        IF(FLAG) WRITE(6,FMT='(A6)') NAME
        IF(FLAG) WRITE(70,FMT='(A6)') NAME               ! Record maplets used
